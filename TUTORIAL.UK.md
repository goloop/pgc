...
застосованого файлу зупиняє наступний запуск помилкою, а не запускає файл
повторно (пиши наступну міграцію - схема рухається лише вперед).
`pgc migrate status` показує, що застосовано, а що чекає, і завершується з
ненульовим кодом, коли щось потребує уваги. Нумеровані імена (`001_...`,
...
точно дорівнює повному набору колонок таблиці - поруч, у будь-якому
порядку (`a.*` завжди підходить). Повторюй анотацію, щоб вкласти кілька
таблиць; іменуй поля через `as` - обов'язково, коли вкладаєш ту саму
таблицю двічі.
...
`pgc check` - середина: компілює кожен запит, нічого не пише і падає, коли
згенерований пакет на диску відрізняється від результату.
