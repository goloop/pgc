...
- **Зовнішні джойни**: каталог і далі повідомляє колонки приєднаної
  ...
  Попередження змовкає, щойно запит десь заявляє nullability своїх
  результатних колонок - достатньо одного `-- override: <колонка> nullable`
  чи `notnull`. Це нагадування подумати про зовнішню сторону, а не
  поколонкова перевірка: без парсингу SQL pgc не знає, які колонки звідти.
  А коли ти вже все розписав, повторювати це щоразу - лише привчати себе
  прогортати попередження, разом із тими запитами, яким воно ще потрібне.
